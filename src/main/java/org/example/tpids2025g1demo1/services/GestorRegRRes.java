...
public class GestorRegRRes implements IAgregado {
    ...
    // La lista de eventos será un array de diccionarios, cada uno de los cuales
    // tendrá evento y datos
    ...
    public IIterador crearIterador(Object[] elementos) {
        ...
        // crea una instancia de un iterador concreto que permitirá recorrer eventos
        // sísmicos
    }
    ...
    public List<String> nuevaRevisionES() {
        ...
        // Se crea la lista de nuevo (estará vacía y no con eventos de iteraciones
        // anteriores)
        ...
        if (!listaESNoRevisados.isEmpty()) { // Verifica que haya eventos para revisar
            ...
            // Arma un array con los datos de los eventos formateados para pasarlo a la
            // pantalla
    ...
    public void buscarESNoRevisados() { // Metodo para buscar los ES auto detectados que están pendientes de revisión
        // Pedimos un arreglo tipado EventoSismico[].Con toArray(new EventoSismico[0])
        // la lista crea y devuelve un arreglo del tamaño justo y del tipo correcto.
        ...
        while (!iterador.haTerminado()) {
            ...
            if (evento != null) { // Chequea el estado del evento
                Map<String, Object> diccionarioEvento = new HashMap<>(); // Nuevo diccionario que va a tener los eventos
                                                                         // y sus datos
    ...
    public List<String> tomarSeleccionES(String eventoSelecc) {
        for (Map<String, Object> diccEvento : listaESNoRevisados) { // Busca el evento correspondiente en la lista de
            ...
            if (datosStr.equals(eventoSelecc)) { // Compara el texto armado con el que paso la pantalla
                ...
                // Lo vuelve a buscar en la db para tener todos sus datos (relaciones) cargados
                eventoSismicoSeleccionado = eventoSismicoRepository.fetchComplete(evento)
        ...
        this.bloquearEventoSismico(); // Marca el evento como bloqueado para que nadie más lo modifique mientras se
                                      // revisa
    ...
    public void bloquearEventoSismico() { // Bloquea el evento sismico seleccionado
        this.ultimoCambioDeEstado = this.eventoSismicoSeleccionado.revisar(fechaHoraActual, estadoBloqueadoEnRevision,
                empleadoLogueado);
        // Invoca el metodo para cambiar el estado del evento y almacena el cambio de
        // estado que se crea
    ...
    @Transactional
    public String tomarSeleccionResultado(String seleccion, String evento) { // toma la acción a realizar y lleva a cabo
                                                                             // el flujo correspondiente
        this.seleccionResultado = seleccion;
        eventoSismicoSeleccionado = eventoSismicoRepository.fetchComplete(eventoSismicoSeleccionado)
                .orElseThrow(() -> new IllegalArgumentException("No se encontró evento sísmico"));
        ...
        if (this.validarDatosMinimos()) { // valida que se tengan los datos mínimos para poder registrar el resultado
            ...
            switch (seleccionResultado) {
                case "Rechazado": // Si se rechaza, busca al estado 'Rechazado' y rechaza al evento sismico,
                    ...
                    break;
                case "Confirmado": // Si se confirma, busca al estado 'Confirmado' y confirma el evento sismico,
                                   // llamando a los metodos correspondientes
            ...
            // Este flujo no se implementa completo, se pone un retorno para cumplir con la
    ...
    public void rechazarEventoSismico() { // Rechaza el evento sismico seleccionado
        this.eventoSismicoSeleccionado.rechazar(fechaHoraActual, estadoRechazado, empleadoLogueado,
                ultimoCambioDeEstado); // Llama al metodo rechazar()
        // Persistir el evento junto a su nuevo cambio de estado
        this.eventoSismicoSeleccionado = this.eventoSismicoRepository.save(this.eventoSismicoSeleccionado);
    }
    ...
    public void confirmarEventoSismico() { // Confirma el evento sismico seleccionado
        this.eventoSismicoSeleccionado.confirmar(fechaHoraActual, estadoConfirmado, empleadoLogueado,
                ultimoCambioDeEstado); // Llama al metodo confirmar()
        this.notificarConfirmacionAInteresados(); // se notifica a los interesados de la confirmación de un evento
                                                  // sísmico
        // Persistir el evento junto a su nuevo cambio de estado
        this.eventoSismicoSeleccionado = this.eventoSismicoRepository.save(this.eventoSismicoSeleccionado);
    }
    ...
    @SuppressWarnings("unchecked")
    public boolean validarDatosMinimos() { // Valida que exista magnitud, alcance y origen de generación del evento y
                                           // que se haya seleccionado una accion
        float magnitud = 0;
